...
public class ProfileController {
    ...
    public ResponseEntity<ProfileResponse> updateProfile(Authentication authentication,
        ...
        String email = authentication.getName();  // Текущий email
        User updatedUser = userService.updateProfile(email, request);
        ProfileResponse profile = userService.getProfileByEmail(updatedUser.getEmail());  // Верни обновлённый с дешифровкой
        return ResponseEntity.ok(profile);
    }
}